...
FindGate::FindGate(int next, int swayRange)
{
    SWAY_MAX = swayRange;
    nextMode = next;
    currentMode = 0;
    ...
    initialDepthStarted = false;
    initialDepthComplete = false;
...
double FindGate::findTimeDifference(TimePoint end, TimePoint start)
{
    return ((double)std::chrono::duration_cast<std::chrono::milliseconds>(end-start).count())*0.001;
}
void FindGate::findNext()
{
